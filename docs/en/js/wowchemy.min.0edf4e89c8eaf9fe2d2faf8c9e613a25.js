(() => {
  ...
  $(document).ready(function() {
    ...
    $("figure").each(function() {
      var e, f, j, b, g, h, i, c, d, k, l;
      if ($(this).attr("class") == "no-photoswipe")
        return true;
      e = $(this), f = e.find("a"), j = e.find("img"), b = f.attr("href"), g = j.attr("alt"), h = j.attr("src"), f.data("size") ? (i = f.data("size").split("x"), c = {src: b, w: i[0], h: i[1], title: g, msrc: h}, console.log("Using pre-defined dimensions for " + b)) : (c = {src: b, w: 800, h: 600, title: g, msrc: h}, console.log("Using default dimensions for " + b), d = new Image(), d.src = b, k = setInterval(function() {
        var a2 = d.naturalWidth, b2 = d.naturalHeight;
        a2 && b2 && (clearInterval(k), c.w = a2, c.h = b2, console.log("Got actual dimensions for " + d.src));
      }, 30)), l = a.length, a.push(c), e.on("click", function(d2) {
        var b2, c2;
        d2.preventDefault(), b2 = $(".pswp")[0], c2 = {index: l, bgOpacity: 0.8, showHideOpacity: true}, new PhotoSwipe(b2, PhotoSwipeUI_Default, a, c2).init();
      });
    });
  });
})();
